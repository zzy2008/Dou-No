...
def main():
    ...
    user_id = 000000
    userCookie = """"""
    user_cookies = addCookie(userCookie)
    token = ""
    ...
    if choice == 1:
        ...
        watched_to_notion = MovieList(
            user_cookies=user_cookies, database_id=database_id, status="看过", token=token
        )
        ...
        watched_to_notion = MovieList(
            user_cookies=user_cookies, database_id=database_id, status="想看", token=token
        )
    ...
    if notion_page_url[0:22] == "https://www.notion.so/":
        if notion_page_url.count("?v=") == 0:
            if notion_page_url.count("#") == 0:
                page_id = (
                    notion_page_url[
                        len(notion_page_url) - 32: len(notion_page_url) - 24
                    ]
                    + "-"
                    + notion_page_url[
                        len(notion_page_url) - 24: len(notion_page_url) - 20
                    ]
                    + "-"
                    + notion_page_url[
                        len(notion_page_url) - 20: len(notion_page_url) - 16
                    ]
                    + "-"
                    + notion_page_url[
                        len(notion_page_url) - 16: len(notion_page_url) - 12
                    ]
                    + "-"
                    + notion_page_url[len(notion_page_url) -
                                      12: len(notion_page_url)]
                )
                return page_id
            elif notion_page_url.count("#") != 0:
                last_hash = notion_page_url.index("#")
                page_id = (
                    notion_page_url[last_hash - 32: last_hash - 24]
                    + "-"
...
def addCookie(userCookie):
    # cookie input
    cookieStr = userCookie

    if cookieStr[0:3] == 'll=':
        cookieDict = {}
        cookieList = cookieStr.replace('"', '').split("; ")
        for kv in cookieList:
            name, value = kv.split('=', 1)
            cookieDict[f"{name}"] = value
