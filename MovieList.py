...
class MovieList:
    ...
    def req(self, url, start_number):
        r1 = requests.request(
            "GET", url=url, headers=self.headers, cookies=self.cookies)
        page_number = start_number / 15 + 1
        if r1.status_code == 200:
            print(f'第{int(page_number)}页访问成功，正在进行同步……')
            list_soup = BeautifulSoup(r1.text, 'html.parser')
            ...
            for movie in movies_list:
                ...
                try:
                    rating = movie.find(
                        "span", class_=re.compile("rating"))["class"]
                    if rating == ['rating1-t']:
                        movie_rate = "⭐️"
                ...
                # 条目短评
                try:
                    movie_comment = movie.find(
                        "span", class_="comment").get_text()
                ...
                r2 = requests.request(
                    "GET", url=movie_link, headers=self.headers, cookies=self.cookies)
                if r2.status_code == 200:
                    ...
                    try:
                        movie_director = []
                        directors = detail_soup.find(
                            "span", class_="attrs").get_text().split(" / ")
                    ...
                    try:
                        movie_country = []
                        countries = detail_soup.find("span", class_="pl", text=re.compile(
                            "制片国家")).next_sibling.lstrip().split(" / ")
                    ...
                    # 条目imdb链接
                    try:
                        imdb_id = detail_soup.find(
                            "span", class_="pl", text=re.compile("IMDb")).next_sibling.lstrip()
                    ...
                    # 条目封面
                    try:
                        poster_origin_url = detail_soup.find(
                            "img", rel="v:image")["src"]
                        poster_l_url = poster_origin_url.replace(
                            "s_ratio_poster", "l")
                    ...
                    try:
                        movie_year = {}
                        find_year = detail_soup.find(
                            "span", class_="year").get_text().replace("(", "").replace(")", "")
